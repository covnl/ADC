/*
 * File: C:\ECOTRON\Product\SW\SEA\slprj\ert\_sharedutils\intrp1d_u8u8u16n15_lca_f.c
 *
 ...
 *const table, uint32_T maxIndex)
{
  ...
  if((uint32_T)bpIndex == maxIndex) {
    ...
    if(yR_0d0 >= yL_0d0) {
      y = (uint8_T)((uint16_T)yL_0d0 + (uint16_T)(uint8_T)(((uint32_T)frac *
        (uint32_T)(uint8_T)((uint16_T)yR_0d0 - (uint16_T)yL_0d0)) >> 15));
    } else {
      y = (uint8_T)((uint16_T)yL_0d0 - (uint16_T)(uint8_T)(((uint32_T)frac *
        (uint32_T)(uint8_T)((uint16_T)yL_0d0 - (uint16_T)yR_0d0)) >> 15));
    }
  }
  return y;
}
