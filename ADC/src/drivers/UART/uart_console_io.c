...
static uint32_t bUartConsoleInitialized = 0;
...
/************************************************************************/
#pragma overload int_t __write_console(__file_handle handle, uchar_t * buffer, size_t * count);
...
UARTError __init_uart_console(void)
{
	...
	if (initialized == 0) {
		...
		//err = vfnUartInit(&LINFlexD_2, kBaud115200);
		err = InitializeUART( kBaud115200 );

		if (err == kUARTNoError) {
			initialized = 1;
			bUartConsoleInitialized = 1;
